...
public class ProcessMemory
{
    ...
    public ProcessMemory ( Process proc, int processhandle )
    {
        this.processID = proc.Id;
        this.ProcessName = proc.ProcessName;
        this.MyProcess = proc;
        // this.myProcessModule = proc.MainModule;
    ...
    public int DllImageAddress ( string dllname )
    {
        ...
        {
            if ( processModule.FileName.ToLower( ).IndexOf(dllname.ToLower( )) > -1 )
    ...
    [DllImport( "kernel32.dll" )]
    public static extern uint ResumeThread ( int hThread );

    [DllImport( "kernel32.dll" )]
    public static extern uint SuspendThread ( int hThread );

    public void SuspendProcess ( )
    {
        if ( this.MyProcess.Threads.Count > 0 )
            SuspendThread( this.processHandle );
    }
    public void ResumeProcess ( )
    {
        if ( this.MyProcess.Threads.Count > 0 )
            ResumeThread( this.processHandle );
    }
